...
SERVICE_CAPS = GenomeAnnotationAPI
SPEC_FILE = GenomeAnnotationAPI.spec
URL = https://kbase.us/services/genomeannotationapi
DIR = $(shell pwd)
LIB_DIR = lib
SCRIPTS_DIR = scripts
TEST_DIR = test
...
build-test-script:
	...
	echo 'set -e' >> $(TEST_DIR)/$(TEST_SCRIPT_NAME)
	echo 'set -x' >> $(TEST_DIR)/$(TEST_SCRIPT_NAME)
